...
class TFProcess:
    ...
    def init_net(self, next_batch):
        ...
        self.z_ = next_batch[1] # tf.placeholder(tf.float32, [None, 1])
        self.batch_norm_count = 0
        self.z_conv = self.construct_net(self.x)
        ...
        loss = self.mse_loss + self.reg_term
    ...
    def process(self, batch_size):
        ...
        mse_loss, reg_term, _, _ = self.session.run(
            [self.mse_loss, self.reg_term, self.train_op,
                self.next_batch],
        ...
        if steps % 1000 == 0:
            ...
            print("step {}, mse={:g} reg={:g} ({:g} pos/s)".format(
                steps, avg_mse_loss, avg_reg_term,
                speed))
            train_summaries = tf.Summary(value=[
                tf.Summary.Value(tag="MSE Loss", simple_value=avg_mse_loss)])
            self.train_writer.add_summary(train_summaries, steps)
            self.time_start = time_end
            self.avg_mse_loss, self.avg_reg_term = [], []
        if steps % 8000 == 0:
            sum_mse = 0
            test_batches = 800
            for _ in range(0, test_batches):
                test_mse, _ = self.session.run(
                    [self.mse_loss,
                     self.next_batch],
    ...
    def construct_net(self, planes):
        ...
        conv_pol = self.conv_block(flow, filter_size=1,
                                   ...
                                   output_channels=2)
        h_conv_pol_flat = tf.reshape(conv_pol, [-1, 2*BOARD_SQ])
        W_fc1 = weight_variable([2 * BOARD_SQ, BOARD_SQ + 1])
        b_fc1 = bias_variable([BOARD_SQ + 1])
        ...
        conv_val = self.conv_block(flow, filter_size=1,
                                   input_channels=RESIDUAL_FILTERS,
                                   output_channels=1)
        h_conv_val_flat = tf.reshape(conv_val, [-1, BOARD_SQ])
        W_fc2 = weight_variable([BOARD_SQ, 256])
